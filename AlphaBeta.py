...
searchTime = 0.0 #var to store time to search in alpha_beta method
...
def playerMove():
    ...
    playerMoveDepth = playerMoveDepth + 1
    return

#compMove: uses alpha_beta to find the best position with best score by checking every state in the tree 
...
def compMove():
    ...
    for key in board.keys():
        if (board[key] == ' '):
            board[key] = bot
            score = alpha_beta(board, alpha, beta, 0, False)
            board[key] = ' '
            if (score > bestScore):
                bestScore = score
                bestMove = key
...
#alpha_beta: finds the best position with best score by checking every state in the tree 
# by comparing the scores of positions
def alpha_beta(board, alpha, beta, depth, isMaximizing):
    global searchTime
    global numOfNodes
    global bestMoveNode
    ...
    if (isMaximizing): #Maximizing: find the best highest score (tries to maximze score to win) by checking and comparing
                        #scores for every state and eliminates states (prouning) if condition alpha >= beta Applies
        start_time = datetime.datetime.now()
        bestScore = -100
        for key in board.keys():
